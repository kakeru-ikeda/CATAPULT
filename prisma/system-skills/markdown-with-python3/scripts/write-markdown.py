#!/usr/bin/env python3
"""Write a Markdown file from one or more chunk files (or stdin).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write markdown file with UTF-8 using one or more input files or stdin. "
            "Multiple --input-file flags are concatenated in order (chunked write)."
        )
    )
