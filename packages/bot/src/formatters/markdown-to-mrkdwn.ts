...
export function markdownToMrkdwn(text: string): string {
  ...
  result = result.replace(/^#{1,6} +(.+)$/gm, (_: string, heading: string) => {
    ...
    return `\x00B${bolds.length - 1}\x00`;
  });
  ...
  // --- 太字を復元 → Slack の *bold* ---
  result = result.replace(/\x00B(\d+)\x00/g, (_: string, i: string) => `*${bolds[Number(i)]}*`);
  ...
  // --- コードブロック・インラインコードを復元 ---
  result = result.replace(/\x00CB(\d+)\x00/g, (_: string, i: string) => codeBlocks[Number(i)] ?? "");
  result = result.replace(/\x00IC(\d+)\x00/g, (_: string, i: string) => inlineCodes[Number(i)] ?? "");
